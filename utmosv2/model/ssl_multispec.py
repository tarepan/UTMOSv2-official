...
class SSLMultiSpecExtModelV1(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        assert cfg.model.ssl_spec.num_classes == 1
        ...
        # Replace sub-module final FC layers with common FC layer
        ...
        self.fc = nn.Linear(ssl_input + spec_long_input + self.num_dataset, 1)

    def forward(self, waves, specs, ds_idc):
        """
        Args:
            waves  :: (B, T)                            - Fixed-length resampled waveforms
            specs  :: (B, Level*Clip, RGB, Freq, Frame) - Spectrograms
            ds_idc :: (B, D)                            - Dataset ID onehot vectors
        """

        # wave-to-feat :: (B, T) & (B, D) -> (B, Feat)
        x1 = self.ssl(waves, torch.zeros(waves.shape[0], self.num_dataset).to(waves.device))

        # spec-to-feat :: (B, Level*Clip, RGB, Freq, Frame) -> (B, Feat)
        x2 = self.spec_long(specs)

        # feat-to-score with dataset index :: (B, Feat) & (B, Feat) & (B, D) -> (B, 1)
        return self.fc(torch.cat([x1, x2, ds_idc], dim=1))
...
class SSLMultiSpecExtModelV2(nn.Module):
    ...
    def forward(self, waves, specs, ds_idc):
        ...
        # spec-to-feat :: (B, Level*Clip, RGB, Freq, Frame) & (B, D) -> (B, Feat)
